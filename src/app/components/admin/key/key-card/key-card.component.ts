...
export class KeyCardComponent implements OnInit {
  ...
  constructor(
    ...
    public keyBehaviorService: KeyBehaviorService,
  ...
  getKeysBySubCategory(): void {
    this.keyService
      .findKeyByKeySubCategory(this.keyBehaviorService.getIdKeySubCategory().id)
      .subscribe((resp) => {
  ...
  openAddKeyDialog(): void {
    DialogUtil.openDialog(
      ...
      DialogOptions.setDialogConfig({
        position: { top: '1%' },
        data: {
          idKeySubCategory: this.keyBehaviorService.getIdKeySubCategory(),
        },
        width: '30%',
        maxHeight: '90vh',
      }),
